...
We see that the average accuracy from the K-fold technique is ~99%.  This implies the model is overfitting for the training data set, which we knew would be a consequence of selecting Random Forest as our model.  A 99% accurcy would also imply that the out of sample error (as said in the lecture videos, larger than the in-sample error), could be anywhere between 1.1% to 100%.  

However, since there are only 5 classes that can be predicted, it is unlikely that the error is high.  An estimate of the out of sample error would be 5%.    
